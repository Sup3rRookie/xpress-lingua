...
- `scripts/render-ja.py` renders Japanese audio (survival phrases, JLPT words, example sentences) with MeloTTS as mp3, with a peak-volume gate that drops silent synthesis. `scripts/prune-silent-ja.py` scans existing clips and records unvoiceable ids in `src/data/ja-audio-suspect.json` so those cards fall back to browser TTS
...
| 0.16 - 0.17 | Travel phrases with native audio, HSK gloss cleanup, native Japanese audio | done |
